...
function Header() {
  ...
  return (
    <div className='fixed h-14 bg-whit top-0 w-full bg-white flex items-center justify-between'>
      ...
      >
        <img className="w-10 h-10 border-4 shadow-md border-neutral-200 bg-barnRed rounded-full" src={user?.photo}></img>
      ...
      >
        <MenuItem onClick={() => {
          navigate("/profile");
          setAnchorEl(null);
        }} style={{ margin: 4, borderRadius: 4, fontWeight: 600 }}>Hồ sơ</MenuItem>
        <MenuItem onClick={() => { eraseCookie("token"); navigate("login") }} style={{ margin: 4, borderRadius: 4, backgroundColor: "#780000", color: "white", fontWeight: "600" }}>Đăng xuất</MenuItem>
